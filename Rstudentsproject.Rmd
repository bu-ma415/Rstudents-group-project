...
```
For this part of the analysis, we are looking at which months food violations tend to peak.The first plot shows the distribution of food inspection violations across all cities over the course of the year. Violations tend to peak in March and October, with October having the highest total number of violations. The second plot breaks the data down by city. Each city follows a similar pattern, with peaks in March and October, consistent with the overall trend observed in the first plot.
...
```
For this part of the analysis, we are looking at which seasons food violations tend to peak. In the third plot, it shows the distribution of food inspection violations across all cities over the seasons. Violations tend to peak in spring with fall being season with the second most amount of food violations. The fourth plot breaks it down by city. Each city follows a similar pattern, with spring being the season with the most violations, consistent with the overall trend observed in the first plot.


```{r}
# Top 3 Violations by season overall
top3_seasonal <- monthly_season |>
  filter(violation_desc %in% top_violations$violation_desc) |>
  group_by(violation_desc, season) |>
  summarise(count = n(), .groups = "drop")|>
  mutate(
    violation_short = case_when(
      violation_desc == "non-food contact surfaces clean" ~ "Non-Food Surfaces Dirty",
      
      violation_desc == "improper maintenance of walls/ceilings" ~ "Walls & Ceilings Damage",
      
      violation_desc == "non-food contact surface or equipment made of unacceptable material, not kept clean, or not properly sealed, raised, spaced or movable to allow accessibility for cleaning on all sides, above and underneath the unit." 
        ~ "Non-Food Equipment Unclean",

      TRUE ~ violation_desc
    ),
    violation_short = factor(
      violation_short,
      levels = c(
        "Non-Food Surfaces Dirty",
        "Walls & Ceilings Damage",
        "Non-Food Equipment Unclean"
      )
    )
  )

ggplot(top3_seasonal,
       aes(x = season, y = count, fill = season)) +
  geom_col() +
  facet_wrap(~ violation_short, scales = "free_y", nrow = 2) +
  labs(
    title = "Seasonal Distribution of Top 3 Violations",
    x = "Season",
    y = "Number of Violations"
  )
```
The above plot displays the distribution of each violation over the different seasons. For all three violations spring has the overall most violations with fall being the second season with the most violations. This, similarly to the plots showing the distribution of each violation over the course of the year, suggests possible seasonal effects on inspection results or restaurant practices.  
...
```
After examining overall and city-level monthly and seasonal trends, we analyzed each violation category separately to determine when different types of violations tend to peak. The figure shows that most categories reach their highest levels in October, suggesting a strong fall seasonal effect across multiple types of food safety risks.

An important exception is Employee Hygiene, which peaks in April rather than October. This may reflect seasonal staffing changes, onboarding of new employees in the spring, or changes in enforcement focus. Overall, this suggests that while many violations follow a common seasonal pattern, some categories exhibit distinct timing, which could be useful for targeted inspections and interventions.
...
```
The figure above shows the distribution for each violation category during each season. The figure shows that most categories reach their highest levels in spring, which is interesting to note as the monthly breakdown showed that most of the violations peaked in month of October. This suggests that even though the most violation occur in October, violations build up during the spring time.

An important exception is Pest Control and Food Temperatrure. Pest control violations peak in the fall, likely because colder outdoor temperatures drive rodents and insects indoors, increasing the risk of infestations in food establishments. Food temperature violations remain relatively consistent across seasons, with a slight increase in the winter, which may be related to increased reheating practices and greater reliance on hot food holding during colder months. Overall, these trends suggest that food safety risks fluctuate seasonally and differently by violation type, reinforcing the importance of timing inspections to periods of elevated risk.
